...
int consume_i32(const unsigned char*& curpointer) {
  int x = read_i32(curpointer);
  curpointer += 4;
  return x;
}

std::string consume_string(const unsigned char*& curpointer) {
  int size = consume_i32(curpointer);
  std::string x((const char *)curpointer, size - 1);
  curpointer += size;
  return x;
}
...
void HIKScript::LoadHikFile(System& system, const fs::path& file) {
  ...
  int file_size = 0;
  std::unique_ptr<unsigned char[]> hik_data;
  ...
  const unsigned char* curpointer = hik_data.get();
  const unsigned char* endpointer = hik_data.get() + file_size;
  ...
  for (auto& p_layer = layers_.begin(); p_layer != layers_.end(); ++p_layer) {
	Layer& layer = *p_layer;
    //for (Animation& animation : layer.animations) {
	auto& animations = layer.animations;
	for (auto& p_animation = animations.begin(); p_animation != animations.end(); ++p_animation) {
