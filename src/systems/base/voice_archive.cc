...
// static
const unsigned char* VoiceSample::MakeWavHeader(int rate, int ch, int bps, int size) {
  static unsigned char header[0x2c];
  memcpy(header, (const unsigned char*)orig_header, 0x2c);
...
void VoiceArchive::ReadVisualArtsTable(boost::filesystem::path file,
  ...
  for (int i = 0; i < table_len; ++i) {
	ifs.read((char *)head, entry_length);
