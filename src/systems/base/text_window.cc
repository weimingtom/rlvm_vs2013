...
#else
      std::string temp = GetKeyParts.at(3);
      int slot = atoi(temp.c_str());
#endif
...
void TextWindow::Render(std::ostream* tree) {
  ...
  if (text_surface && is_visible()) {
    ...
    if (in_selection_mode()) {
	  ...
	  for (auto& p_sel = selections_.begin(); p_sel != selections_.end(); ++p_sel) {
		unique_ptr<SelectionElement>& e = *p_sel;
		e->Render();;
	  }
    } else {
  ...
  if (in_selection_mode()) {
    ...
    bool found =
	  ...
	  */
	  ...
	  for (auto& p_sel = selections_.begin(); p_sel != selections_.end(); ++p_sel) {
			unique_ptr<SelectionElement>& e = *p_sel;
			if (e->HandleMouseClick(pos, pressed)) {
				found = true;
				break;
			}
		}
