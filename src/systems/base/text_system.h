...
class TextSystem : public EventListener {
 ...
 public:
  ...
  virtual std::shared_ptr<TextWindow> GetTextWindow(int text_window_number)/* = 0*/{return 0;} //;
  ...
  virtual Size RenderGlyphOnto(
	  ...
	  const std::shared_ptr<Surface>& destination)/* = 0*/{return Size();}//;

  virtual int GetCharWidth(int size, uint16_t codepoint)/* = 0*/ {return 0;}//;
