...
SimpleFrameCounter::SimpleFrameCounter(EventSystem& es,
  ...
  change_interval_ = float(milliseconds) / std::abs(frame_max - frame_min);
}
...
TurnFrameCounter::TurnFrameCounter(EventSystem& es,
    ...
    : FrameCounter(es, frame_min, frame_max, milliseconds),
      time_at_last_check_(es.GetTicks()) {
  change_interval_ = int(float(milliseconds) / std::abs(frame_max - frame_min));
...
int AcceleratingFrameCounter::ReadFrame() {
  if (is_active_) {
    float base_interval = float(total_time_) / std::abs(max_value_ - min_value_);
    float cur_time =
