...
SoundSystem::SoundSystem(System& system)
...
#else
    int entry_number = atoi(raw_number.c_str());
#endif
  ...
  for (; koeonoff != end; ++koeonoff) {
...
#else
    std::string temp = keyparts.at(1);
    int usekoe_id = atoi(temp.c_str());
#endif
    ...
    if (unprocessed_koeids.find('(') != std::string::npos) {
	  ...
	  for (auto& p_string_id = string_koeplay_ids.begin(); p_string_id != string_koeplay_ids.end(); ++p_string_id) {
		std::string const& string_id = *p_string_id;
#ifndef __MINGW32__
        koeplay_ids.push_back(std::stoi(string_id));
#else
        koeplay_ids.push_back(atoi(string_id.c_str()));
#endif        
      }
    } else {
#ifndef __MINGW32__
...
#else
      koeplay_ids.push_back(atoi(unprocessed_koeids.c_str()));
#endif    
    }

    int onoff = (keyparts.at(3) == "ON") ? 1 : 0;
    //for (int id : koeplay_ids) {
	for (auto& p_id = koeplay_ids.begin(); p_id != koeplay_ids.end(); ++p_id) {
	  int id = *p_id;
