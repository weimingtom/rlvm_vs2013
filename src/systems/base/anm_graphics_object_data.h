...
class AnmGraphicsObjectData : public GraphicsObjectData {
 ...
 private:
  ...
  bool TestFileMagic(std::unique_ptr<unsigned char[]>& anm_data);
  void ReadIntegerList(const unsigned char* start,
                       int offset,
                       int iterations,
                       std::vector<std::vector<int>>& dest);
  void LoadAnmFileFromData(const std::unique_ptr<unsigned char[]>& anm_data);
