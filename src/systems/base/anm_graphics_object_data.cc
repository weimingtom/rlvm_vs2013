...
static const unsigned char ANM_MAGIC[ANM_MAGIC_SIZE] = {'A', 'N', 'M', '3', '2', 0,
                                               0,   0,   0,   1,   0,   0};
...
bool AnmGraphicsObjectData::TestFileMagic(std::unique_ptr<unsigned char[]>& anm_data) {
...
void AnmGraphicsObjectData::LoadAnmFile() {
  ...
  int file_size = 0;
  std::unique_ptr<unsigned char[]> anm_data;
...
void AnmGraphicsObjectData::LoadAnmFileFromData(
    const std::unique_ptr<unsigned char[]>& anm_data) {
  const unsigned char* data = anm_data.get();
  ...
  string raw_file_name = (const char *)(data + 0x1c);
  image_ = system_.graphics().GetSurfaceNamed(raw_file_name);
  image_->EnsureUploaded();

  // Read the frame list
  const unsigned char* buf = data + 0xb8;
...
void AnmGraphicsObjectData::ReadIntegerList(
  ...
  for (int i = 0; i < iterations; ++i) {
    ...
    const unsigned char* tmpbuf = start + 8;
