...
CGMTable::CGMTable(Gameexe& gameexe) {
  ...
  if (strncmp((const char *)data.get(), "CGTABLE", 7) != 0) {
    std::ostringstream oss;
  ...
  std::unique_ptr<unsigned char[]> dest_orig(new unsigned char[dest_size + 1024]);
  unsigned char* dest = dest_orig.get();
  unsigned char* src = data.get() + 0x28;
  ARCINFO::Extract2k(dest, src, dest + dest_size, data.get() + size);
  dest = dest_orig.get();
  for (int i = 0; i < cgm_size; i++) {
    unsigned char* s = dest + i * record_size;
    int n = read_little_endian_int(dest + i * record_size + 32);
    cgm_info_[(const char *)s] = n;
  }
}
