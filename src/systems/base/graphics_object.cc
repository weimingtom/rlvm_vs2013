...
GraphicsObject::GraphicsObject(const GraphicsObject& rhs) : impl_(rhs.impl_) {
  ...
  auto& mutators = rhs.object_mutators_;
  for (auto& p_mutator = mutators.begin(); p_mutator != mutators.end(); ++p_mutator) {
    auto const& mutator = *p_mutator;
...
GraphicsObject& GraphicsObject::operator=(const GraphicsObject& obj) {
  ...
  auto& mutators = obj.object_mutators_;
  for (auto& p_mutator = mutators.begin(); p_mutator != mutators.end(); ++p_mutator) {
    auto const& mutator = *p_mutator;
    object_mutators_.emplace_back(mutator->Clone());
  }
...
void GraphicsObject::AddObjectMutator(std::unique_ptr<ObjectMutator> mutator) {
  ...
  for (auto& p_obj = object_mutators_.begin(); p_obj != object_mutators_.end(); ++p_obj) {
	std::unique_ptr<ObjectMutator>& mutator_ptr = *p_obj;
