...
GraphicsSystem::GraphicsObjectSettings::GraphicsObjectSettings(
  ...
  for (; it != end; ++it) {
    ...
    if (poscolon != string::npos) {
        string temp = s.substr(0, poscolon);
      int obj_num_first = atoi(temp.c_str());
      temp = s.substr(poscolon + 1);
      int obj_num_last = atoi(temp.c_str());
      while (obj_num_first <= obj_num_last) {
        object_nums.push_back(obj_num_first++);
      }
    } else {
      object_nums.push_back(atoi(s.c_str()));
    }

    //for (int obj_num : object_nums) {
	for (auto& p_num = object_nums.begin(); p_num != object_nums.end(); ++p_num) {
	  int obj_num = *p_num;
...
void GraphicsSystem::ExecuteGraphicsSystem(RLMachine& machine) {
  ...
  auto& objs = GetForegroundObjects();
  for (auto& p_obj = objs.begin(); p_obj != objs.end(); ++p_obj) {
     GraphicsObject& obj = *p_obj; 
	 obj.Execute(machine);
  }
...
std::shared_ptr<HIKScript> GraphicsSystem::GetHIKScript(
  ...
  for (auto& p_item = preloaded_hik_scripts_.begin(); p_item != preloaded_hik_scripts_.end(); ++p_item) {
    HIKArrayItem& item = *p_item;
    if (item.first == name)
      return item.second;
  }
...
std::shared_ptr<const Surface> GraphicsSystem::GetPreloadedG00(
  ...
  for (auto& p_item = preloaded_g00_.begin(); p_item != preloaded_g00_.end(); ++p_item) {
    G00ArrayItem& item = *p_item;
    if (item.first == name)
      return item.second;
  }
...
void GraphicsSystem::FreeAllObjectData() {
  ...
  auto& items = graphics_object_impl_->foreground_objects;
  for (auto& p_item = items.begin(); p_item != items.end(); ++p_item) {
    GraphicsObject& object = *p_item;
    object.FreeObjectData();
  }

  //for (GraphicsObject& object : graphics_object_impl_->background_objects)
  auto& items2 = graphics_object_impl_->background_objects;
  for (auto& p_item = items2.begin(); p_item != items2.end(); ++p_item) {
    GraphicsObject& object = *p_item;
    object.FreeObjectData();
  }
}
...
void GraphicsSystem::RenderObjects(std::ostream* tree) {
  ...
  std::sort(to_render.begin(), to_render.end(), [](boost::tuple<int, int, int, int, GraphicsObject*>& a, boost::tuple<int, int, int, int, GraphicsObject*>& b) {
