...
std::shared_ptr<const Surface> SDLGraphicsSystem::LoadSurfaceFromFile(
  ...
  std::unique_ptr<unsigned char[]> d(new unsigned char[size + 1]);
  ...
  unsigned char* mem = (unsigned char*)malloc(conv->Width() * conv->Height() * 4 + 1024);
  SDL_Surface* s = 0;
  if (conv->Read(mem)) {
...
#else
    int effect_no = atoi(effect_no_str.c_str());
#endif
