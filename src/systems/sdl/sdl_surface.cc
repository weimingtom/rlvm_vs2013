...
void TransformSurface(SDLSurface* our_surface,
  ...
  // determine position
  unsigned char* p_position = (unsigned char*)surface->pixels;
...
void SDLSurface::uploadTextureIfNeeded() const {
  if (!texture_is_valid_) {
    if (textures_.size() == 0) {
      ...
      //});
	  for (auto& p_texture = textures_.begin(); p_texture != textures_.end(); ++p_texture) {
		  TextureRecord& record = *p_texture;
		  record.reupload(surface_, dirty_rectangle_);
	  }
    }
...
void SDLSurface::GetDCPixel(const Point& pos, int& r, int& g, int& b) const {
  ...
  // determine position
  unsigned char* p_position = (unsigned char*)surface_->pixels;
