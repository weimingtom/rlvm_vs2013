...
std::unique_ptr<unsigned char[]> Texture::s_upload_buffer;
...
Texture::Texture(SDL_Surface* surface,
  ...
  if (w == total_width_ && h == total_height_) {
    ...
    unsigned char* pixel_data = uploadBuffer(surface->format->BytesPerPixel * w * h);
    unsigned char* cur_dst_ptr = pixel_data;

    SDL_LockSurface(surface);
    {
      unsigned char* cur_src_ptr = (unsigned char*)surface->pixels;
...
unsigned char* Texture::uploadBuffer(unsigned int size) {
  if (!s_upload_buffer || size > s_upload_buffer_size) {
    s_upload_buffer.reset(new unsigned char[size]);
