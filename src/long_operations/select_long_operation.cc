...
SelectLongOperation::SelectLongOperation(RLMachine& machine,
  ...
  auto& params = commandElement.raw_params();
  for (auto& p_param = params.begin(); p_param != params.end(); ++p_param) {
	SelectElement::Param const& param = *p_param; 
    Option o;
    o.shown = true;
    o.enabled = true;
    o.use_colour = false;
	...
	auto& conditions = param.cond_parsed;
	for (auto& p_condition = conditions.begin(); p_condition != conditions.end(); ++p_condition) {
      ...
      switch (condition.effect) {
        ...
        case SelectElement::OPTION_HIDE: {
          ...
          if (condition.condition != "") {
			  const unsigned char* location = (const unsigned char *)condition.condition.c_str();
        ...
        case SelectElement::OPTION_TITLE: {
          ...
          if (!enabled && condition.effect_argument != "") {
			const unsigned char* location = (const unsigned char *)condition.effect_argument.c_str();
...
std::vector<std::string> SelectLongOperation::GetOptions() const {
  ...
  for (auto& p_option = options_.begin(); p_option != options_.end(); ++p_option) {
    Option const& option = *p_option;
