...
ButtonObjectSelectLongOperation::ButtonObjectSelectLongOperation(
  ...
  auto& objs = graphics.GetForegroundObjects();
  for (auto& p_obj = objs.begin(); p_obj != objs.end(); ++p_obj) {
    ...
    if (obj.IsButton() && obj.GetButtonGroup() == group_) {
      ...
      if (parent) {
		...
		auto& objs2 = parent->objects();
		for (auto& p_child = objs2.begin(); p_child != objs2.end(); ++p_child) {
		  auto& child = *p_child;
  ...
  for (auto & p_button_pair = buttons_.begin(); p_button_pair != buttons_.end(); ++p_button_pair) {
	ButtonPair& button_pair = *p_button_pair;
