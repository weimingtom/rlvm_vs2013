...
void CopyOneShiftJisCharacter(const unsigned char*& str, string& output) {
...
bool ReadFullwidthLatinLetter(const unsigned char*& str, string& output) {
  // The fullwidth uppercase latin characters are 0x8260 through 0x8279.
  if (str[0] == 0x82) {
    if (str[1] == 0) {
      throw rlvm::Exception("Malformed Shift_JIS string!");
    } else if (str[1] >= 0x60 && str[1] <= 0x79) {
      unsigned char printable = str[1] - 0x1F;
      output += printable;
      str += 2;
      return true;
    }
