...
#else
  auto& result = property_list_->begin();
  for (; result != property_list_->end(); ++result) {
	Property& p = *result;
	if (p.first == property) {
		break;
	}
...
StrConstant_T::type StrConstant_T::getData(
  ...
  // 'SOMEFILE' around. LRUCache and string memory now point to the same unsigned char*
  // buffer.
  //
...
void RLOp_SpecialCase::ParseParameters(
  ...
  for (auto& p_parameter = input.begin(); p_parameter != input.end(); ++p_parameter) {
	auto const& parameter = *p_parameter;
	const unsigned char* src = (const unsigned char *)parameter.c_str();
