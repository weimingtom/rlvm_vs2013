  ...
  if (halted() == true) {
    ...
    try {
      if (call_stack_.back().frame_type == StackFrame::TYPE_LONGOP) {
		...
		for (auto& p_action = delayed_modifications_.begin(); p_action != delayed_modifications_.end(); ++p_action) {
		  auto const& action = *p_action;
          (action)();
        }
...
bool RLMachine::DllLoaded(const std::string& name) {
  ...
  for (auto& p_dll = loaded_dlls_.begin(); p_dll != loaded_dlls_.end(); ++p_dll) {
	auto const& dll = *p_dll;
