...
IntMemRef::IntMemRef(unsigned char bank_name, int location)
...
IntMemRef::IntMemRef(unsigned char bank_name, const unsigned char* access_str, int location)
    : access_type_(0), location_(location) {
  if (bank_name >= 'A' && bank_name <= 'G')
    memory_bank_ = bank_name - 'A';
  else if (bank_name == 'Z')
  ...
  if (strcmp((const char *)access_str, "") == 0)
    access_type_ = 0;
  else if (strcmp((const char *)access_str, "b") == 0)
    access_type_ = 1;
  else if (strcmp((const char *)access_str, "2b") == 0)
    access_type_ = 2;
  else if (strcmp((const char *)access_str, "4b") == 0)
    access_type_ = 3;
  else if (strcmp((const char *)access_str, "8b") == 0)
    access_type_ = 4;
  else
...
std::ostream& operator<<(std::ostream& oss,
  ...
  if (bank >= libreallive::INTA_LOCATION && bank <= libreallive::INTG_LOCATION)
    oss << unsigned char('A' + memref.bank());
