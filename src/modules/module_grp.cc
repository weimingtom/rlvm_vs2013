...
void ReplayGraphicsStackCommand(RLMachine& machine,
  ...
  try {
	...
	for (auto& p_command = stack.begin(); p_command != stack.end(); ++p_command) {
	  auto const& command = *p_command;
      if (command != "") {
        ...
        libreallive::BytecodeElement* element =
			...
			(const unsigned char *)command.c_str(), (const unsigned char *)(command.c_str() + command.size()), cdata);
...
void ReplayDepricatedGraphicsStackVector(
  ...
  for (auto& p_frame = gstack.begin(); p_frame != gstack.end(); ++p_frame) {
	auto const& frame = *p_frame;
    try {
