...
inline unsigned char ToUpper(unsigned char x) { return toupper(x); }
...
inline unsigned char ToLower(unsigned char x) { return tolower(x); }

// Impelmentation for the ASCII versions of itoa. Sets |length| |fill|
// characters.
std::string rl_itoa_implementation(int number, int length, unsigned char fill) {
  std::ostringstream ss;
  if (number < 0)
    ss << "-";
  if (length > 0)
    ss << std::setw(length);
  ss << std::right << std::setfill((char)fill) << abs(number);
  return ss.str();
}
...
struct strrsub_0 : public strsub_0 {
  void operator()(RLMachine& machine,
                  ...
                  std::string source,
                  int offsetFromBack) {
	int offset = strcharlen((const unsigned char*)source.c_str()) - offsetFromBack;
...
#if defined(_MSC_VER)
    machine.PerformTextout(std::to_string((__int64)value));
#elif !defined(__MINGW32__)
    machine.PerformTextout(std::to_string(value));
#else
    std::stringstream newstr;
    newstr<<value;
    machine.PerformTextout(newstr.str());    
#endif
  }
};
