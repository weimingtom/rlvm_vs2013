...
GCNMenu::GCNMenu(const std::string& title,
  ...
  for (auto& p_data = buttons_to_display.begin(); p_data != buttons_to_display.end(); ++p_data) {
	const GCNMenuButton& data = *p_data;
    if (data.separator) {
...
#else
	  max_button_size = max(max_button_size, button->getWidth());
#endif
  ...
  for (auto& p_button = buttons.begin(); p_button != buttons.end(); ++p_button) {
	gcn::Button* button = *p_button;
