...
string hantozen_cp932(const string& input, int transformation) {
  ...
  if (transformation == 0) {
    string output;
	const unsigned char* s = (const unsigned char*)input.c_str();
	const unsigned char* end = (const unsigned char*)(input.c_str() + input.size());
...
static int findCharInTable(const unsigned char* str,
                           uint16_t upper_hantozen_table[],
                           unsigned char start,
                           unsigned char end) {
  ...
  if (transformation == 0) {
    ...
    while (s < end) {
      if (shiftjis_lead_byte(*s)) {
        ...
        if (han) {
          output += static_cast<unsigned char>(han);
          s += 2;
        } else {
          // Not an anything.
          CopyOneShiftJisCharacter(s, output);
