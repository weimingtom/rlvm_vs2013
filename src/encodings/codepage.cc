...
void Codepage::JisEncodeString(const unsigned char* src,
								unsigned char* buf,
                               size_t buflen) const {
	std::strncpy((char*)(buf), (const char*)(src), buflen);
}

void Codepage::JisDecodeString(const unsigned char* src,
								unsigned char* buf,
                               size_t buflen) const {
  size_t srclen = std::strlen((const char *)src), i = 0, j = 0;
...
bool Codepage::DbcsDelim(unsigned char* str) const { return false; }
