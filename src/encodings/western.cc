...
bool Cp1252::DbcsDelim(unsigned char* str) const {
  return str[0] == 0x89 &&
...
void Cp1252::JisEncodeString(const unsigned char* src, unsigned char* buf, size_t buflen) const {
  while (*src && buflen--) {
...
std::wstring Cp1252::ConvertString(const std::string& in_string) const {
  ...
  const unsigned char* s = (const unsigned char*)in_string.c_str();
  while (*s)
    rv += Convert(*s++);
